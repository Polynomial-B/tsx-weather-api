...
function App() {
  ...
  function getLocations(options: string) {
      fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${options.trim()}&limit=5&appid=${API_KEY}`)
      .then((res)=>res.json())
  ...
  function getForecast(city: OptionsType) {
      fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${city.lat}&lon=${city.lon}&units=standard&appid=${API_KEY}`)
      .then((res)=>res.json())
  ...
  return <>
    <main className="flex justify-center items-center bg-gradient-to-br from-pink-600 via-fuschia-600 to-purple-600 h-[100vh] w-full">
      {weatherForecast ? "Forecast" : <Search
            location={location}
            options={options}
            onChange={onChange}
